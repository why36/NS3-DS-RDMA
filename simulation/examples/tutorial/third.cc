...
bool clamp_target_rate = false, clamp_target_rate_after_timer = false, send_in_chunks = true, l2_wait_for_ack = false, l2_back_to_zero = false,
     l2_test_read = false;
...
int main(int argc, char *argv[]) {
    ...
    for (uint32_t i = 0; i < flow_num; i++) {
        ...
        while (used_port[port = int(UniformVariable(0, 1).GetValue() * 40000)]) continue;
        used_port[port] = true;
        flowf >> src >> dst >> pg >> maxPacketCount >> start_time >> stop_time;
        NS_ASSERT(n.Get(src)->GetNodeType() == kNodeType::Host && n.Get(dst)->GetNodeType() == kNodeType::Host);
