...
void QbbHelper::SetQueue(std::string type, std::string n1, const AttributeValue &v1, std::string n2, const AttributeValue &v2, std::string n3,
...
void QbbHelper::SetDeviceAttribute(std::string n1, const AttributeValue &v1) { m_deviceFactory.Set(n1, v1); }
...
void QbbHelper::EnableAsciiInternal(Ptr<OutputStreamWrapper> stream, std::string prefix, Ptr<NetDevice> nd, bool explicitFilename) {
    //
...
NetDeviceContainer QbbHelper::Install(NodeContainer c) {
    NS_ASSERT(c.GetN() == 2);
    return Install(c.Get(0), c.Get(1));
}

NetDeviceContainer QbbHelper::Install(Ptr<Node> a, Ptr<Node> b) {
...
NetDeviceContainer QbbHelper::Install(Ptr<Node> a, std::string bName) {
    Ptr<Node> b = Names::Find<Node>(bName);
    return Install(a, b);
}

NetDeviceContainer QbbHelper::Install(std::string aName, Ptr<Node> b) {
    Ptr<Node> a = Names::Find<Node>(aName);
    return Install(a, b);
}

NetDeviceContainer QbbHelper::Install(std::string aName, std::string bName) {
    Ptr<Node> a = Names::Find<Node>(aName);
    Ptr<Node> b = Names::Find<Node>(bName);
    return Install(a, b);
}
...
void QbbHelper::GetTraceFromPacket(TraceFormat &tr, Ptr<QbbNetDevice> dev, Ptr<const Packet> p, uint32_t qidx, Event event, bool hasL2) {
    ...
    tr.nodeType = static_cast<uint8_t>(dev->GetNode()->GetNodeType());
...
void QbbHelper::MacRxDetailCallback(FILE *file, Ptr<QbbNetDevice> dev, Ptr<const Packet> p) { PacketEventCallback(file, dev, p, 0, Recv, true); }
...
void QbbHelper::EnableTracing(FILE *file, NodeContainer node_container) {
    ...
    for (NodeContainer::Iterator i = node_container.Begin(); i != node_container.End(); ++i) {
        ...
        for (uint32_t j = 0; j < node->GetNDevices(); ++j) {
            if (node->GetDevice(j)->IsQbb()) EnableTracingDevice(file, DynamicCast<QbbNetDevice>(node->GetDevice(j)));
        }
    }
}
