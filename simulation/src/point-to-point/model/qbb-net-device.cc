...
void QbbNetDevice::DequeueAndTransmit(void) {
    ...
    Ptr<Packet> p;
    if (m_node->GetNodeType() == kNodeType::Host) {
        ...
        if (p != 0) {
            ...
            NS_LOG_INFO("PAUSE prohibits send at node " << m_node->GetId());
            if (m_node->GetNodeType() == kNodeType::Host &&
                m_qcnEnabled) {  // nothing to send, possibly due to qcn flow control, if so reschedule sending
                Time t = Simulator::GetMaximumSimulationTime();
                for (uint32_t i = 0; i < m_rdmaEQ->GetFlowCount(); i++) {
                    Ptr<RdmaQueuePair> qp = m_rdmaEQ->GetQp(i);
                    if (qp->GetBytesLeft() == 0) continue;
...
void QbbNetDevice::Receive(Ptr<Packet> packet) {
    ...
    if (ch.l3Prot == 0xFE) {  // PFC
        ...
        if (m_node->GetNodeType() == kNodeType::Switch) {  // switch
