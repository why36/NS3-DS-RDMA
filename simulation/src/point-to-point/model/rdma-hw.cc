...
TypeId RdmaHw::GetTypeId(void) {
    static TypeId tid =
        TypeId("ns3::RdmaHw")
            ...
            .AddAttribute("CCType", "IPbased or Flowbased congestion control", UintegerValue(static_cast<uint8_t>(CongestionControlType::FlowBase)),
                          MakeUintegerAccessor(&RdmaHw::m_CCType), MakeUintegerChecker<uint8_t>())
...
#if 1
        std::cout << "Initializing RdmaHw for dev: " << dev << std::endl;
#endif
...
uint32_t RdmaHw::GetNicIdxOfQp(Ptr<RdmaQueuePair> qp) {
    uint64_t dip = qp->m_connectionAttr.dip.Get();
    if (m_rtTable.find(dip) == m_rtTable.end()) {
        NS_ASSERT_MSG(false, "cannot find NICs for the distination IP");
        return 0;
    }
...
Ptr<RdmaQueuePair> RdmaHw::AddQueuePair(const QPConnectionAttr &attr) {
    ...
    } else if (m_cc_mode == 3)  // HPCC
    {
        ...
        if (m_multipleRate) {
            for (uint32_t i = 0; i < IntHeader::maxHop; i++) cc_entity->hp.hopState[i].Rc = m_bps;
        }
...
int RdmaHw::ReceiveCnp(Ptr<Packet> p, CustomHeader &ch) {
    ...
    if (qp->m_CCEntity->m_rate == 0)  // lazy initialization
    {
        ...
        if (m_cc_mode == 1) {
            ...
            if (m_multipleRate) {
                for (uint32_t i = 0; i < IntHeader::maxHop; i++) qp->m_CCEntity->hp.hopState[i].Rc = dev->GetDataRate();
            }
        } else if (m_cc_mode == 7) {
...
UCSeqState RdmaHw::MatchFirstOrOnly(IBHeader ibh) {
    if (ibh.GetOpCode().GetOpCodeOperation() == OpCodeOperation::SEND_ONLY_WITH_IMM ||
        ibh.GetOpCode().GetOpCodeOperation() == OpCodeOperation::SEND_ONLY || ibh.GetOpCode().GetOpCodeOperation() == OpCodeOperation::SEND_FIRST) {
...
void RdmaHw::AddTableEntry(Ipv4Address &dstAddr, uint32_t intf_idx) {
    ...
    if (m_rtTable.find(dip) == m_rtTable.end()) {
        m_rtTable[dip] = std::make_shared<std::vector<int>>();
    }
#if 1
    std::cout << "AddTableEntry: "
              << " src: " << this->m_node->GetObject<Ipv4>()->GetAddress(1, 0) << " dst: " << dstAddr << std::endl;
#endif
...
void RdmaHw::ActiveIncreaseMlx(Ptr<CongestionControlEntity> q) {
    ...
    if (q->mlx.m_targetRate > q->m_max_rate) q->mlx.m_targetRate = q->m_max_rate;
    q->m_rate = (q->m_rate / 2) + (q->mlx.m_targetRate / 2);
#if PRINT_LOG
    printf("(%.3lf %.3lf)\n", q->mlx.m_targetRate.GetBitRate() * 1e-9, q->m_rate.GetBitRate() * 1e-9);
#endif
...
void RdmaHw::HandleAckHp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch) {
    ...
    // update rate
    if (ack_seq > qp->m_CCEntity->hp.m_lastUpdateSeq) {  // if full RTT feedback is ready, do full update
...
void RdmaHw::UpdateRateHp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react) {
    ...
    if (sender->hp.m_lastUpdateSeq == 0) {  // first RTT
        sender->hp.m_lastUpdateSeq = next_seq;
        // store INT
        IntHeader &ih = ch.ack.ih;
        NS_ASSERT(ih.nhop <= IntHeader::maxHop);
        for (uint32_t i = 0; i < ih.nhop; i++) sender->hp.hop[i] = ih.hop[i];
#if PRINT_LOG
        if (print) {
        ...
        if (ih.nhop <= IntHeader::maxHop) {
            ...
            for (uint32_t i = 0; i < ih.nhop; i++) {
...
#if PRINT_LOG
                if (print)
                    printf(" %u(%u) %lu(%lu) %lu(%lu)", ih.hop[i].GetQlen(), sender->hp.hop[i].GetQlen(), ih.hop[i].GetBytes(),
                           sender->hp.hop[i].GetBytes(), ih.hop[i].GetTime(), sender->hp.hop[i].GetTime());
#endif
                ...
                if (!m_multipleRate) {
                    ...
                    if (tau > sender->m_baseRtt) tau = sender->m_baseRtt;
            ...
            if (!m_multipleRate) {
                // for aggregate (single R)
                if (updated_any) {
                    if (dt > sender->m_baseRtt) dt = sender->m_baseRtt;
                    ...
                    if (new_rate > sender->m_max_rate) new_rate = sender->m_max_rate;
#if PRINT_LOG
                    if (print) printf(" u=%.6lf U=%.3lf dt=%u max_c=%.3lf", sender->hp.u, U, dt, max_c);
#endif
#if PRINT_LOG
                ...
                for (uint32_t i = 0; i < ih.nhop; i++) {
                    if (updated[i]) {
                        ...
                        if (new_rate_per_hop[i] > sender->m_max_rate) new_rate_per_hop[i] = sender->m_max_rate;
                        // find min new_rate
                        if (new_rate_per_hop[i] < new_rate) new_rate = new_rate_per_hop[i];
#if PRINT_LOG
                        if (print) printf(" [%u]u=%.6lf c=%.3lf", i, sender->hp.hopState[i].u, c);
#endif
#if PRINT_LOG
                        if (print) printf(" %.3lf->%.3lf", sender->hp.hopState[i].Rc.GetBitRate() * 1e-9, new_rate.GetBitRate() * 1e-9);
#endif
                    } else {
                        if (sender->hp.hopState[i].Rc < new_rate) new_rate = sender->hp.hopState[i].Rc;
                    }
                }
#if PRINT_LOG
                printf("\n");
            ...
            if (updated_any) ChangeRate(sender, new_rate);
            if (!fast_react) {
                if (updated_any) {
        ...
        if (!fast_react) {
            if (next_seq > sender->hp.m_lastUpdateSeq) sender->hp.m_lastUpdateSeq = next_seq;  //+ rand() % 2 * m_mtu;
        }
    }
}
...
void RdmaHw::HandleAckTimely(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch) {
    uint32_t ack_seq = ch.ack.seq;
    // update rate
    if (ack_seq > qp->m_CCEntity->tmly.m_lastUpdateSeq) {  // if full RTT feedback is ready, do full update
...
void RdmaHw::UpdateRateTimely(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool us) {
    ...
    bool print = !us;
    Ptr<CongestionControlEntity> sender = qp->m_CCEntity;
    if (sender->tmly.m_lastUpdateSeq != 0) {  // not first RTT
        ...
        if (inc) {
            ...
            if (sender->m_rate > sender->m_max_rate) sender->m_rate = sender->m_max_rate;
            if (!us) {
...
void RdmaHw::HandleAckDctcp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch) {
    ...
    if (ack_seq > qp->m_CCEntity->dctcp.m_lastUpdateSeq) {  // if full RTT feedback is ready, do alpha update
#if PRINT_LOG
        printf("%lu %s %08x %08x %u %u [%u,%u,%u] %.3lf->", Simulator::Now().GetTimeStep(), "alpha", real_qp->m_connectionAttr.sip.Get(),
               real_qp->m_connectionAttr.dip.Get(), real_qp->m_connectionAttr.sport, real_qp->m_connectionAttr.dport, real_qp->dctcp.m_lastUpdateSeq,
               ch.ack.seq, qp->snd_nxt, qp->dctcp.m_alpha);
#endif
        new_batch = true;
        if (qp->m_CCEntity->dctcp.m_lastUpdateSeq == 0) {  // first RTT
    ...
    if (qp->m_CCEntity->dctcp.m_caState == 1) {
        if (ack_seq > qp->m_CCEntity->dctcp.m_highSeq) qp->m_CCEntity->dctcp.m_caState = 0;
    }
...
void RdmaHw::HandleAckHpPint(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch) {
    ...
    // update rate
    if (ack_seq > qp->m_CCEntity->hpccPint.m_lastUpdateSeq) {  // if full RTT feedback is ready, do full update
...
void RdmaHw::UpdateRateHpPint(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react) {
    uint32_t next_seq = qp->snd_nxt;
    if (qp->m_CCEntity->hpccPint.m_lastUpdateSeq == 0) {  // first RTT
        ...
        if (new_rate < m_minRate) new_rate = m_minRate;
        if (new_rate > qp->m_CCEntity->m_max_rate) new_rate = qp->m_CCEntity->m_max_rate;
        ...
        if (!fast_react) {
            if (next_seq > qp->m_CCEntity->hpccPint.m_lastUpdateSeq) qp->m_CCEntity->hpccPint.m_lastUpdateSeq = next_seq;  //+ rand() % 2 * m_mtu;
        }
    }
}
