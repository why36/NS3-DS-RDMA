...
void RdmaDriver::Init(void) {
...
#if 0
	m_rdma_hw->m_nic.resize(ipv4->GetNInterfaces());
	for (uint32_t i = 0; i < m_rdma_hw->m_nic.size(); i++){
		m_rdma_hw->m_nic[i] = CreateObject<CongestionControlGroup>();
		// share the queue pair group with NIC
		if (ipv4->GetNetDevice(i)->IsQbb()){
			DynamicCast<QbbNetDevice>(ipv4->GetNetDevice(i))->m_rdmaEQ->m_qpGrp = m_rdma_hw->m_nic[i];
		}
	}
#endif
    for (uint32_t i = 0; i < m_node->GetNDevices(); i++) {
        ...
        m_rdma_hw->m_nic.push_back(RdmaInterfaceMgr(dev));
        m_rdma_hw->m_nic.back().qpGrp = CreateObject<QueuePairSet>();
    }
#if 0
...
#endif
    // RdmaHw do setup
    m_rdma_hw->SetNode(m_node);
    m_rdma_hw->Setup(MakeCallback(&RdmaDriver::QpComplete, this));
}

Ptr<RdmaQueuePair> RdmaDriver::AddQueuePair(QPCreateAttribute& create_attr) {
    // TO DO Krayecho Yx: converge this
    return m_rdma_hw->AddQueuePair(create_attr.conAttr);
}
