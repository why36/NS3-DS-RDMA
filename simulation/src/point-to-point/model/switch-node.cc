...
TypeId SwitchNode::GetTypeId(void) {
    static TypeId tid =
        TypeId("ns3::SwitchNode")
            ...
            .AddAttribute("EcnEnabled", "Enable ECN marking.", BooleanValue(false), MakeBooleanAccessor(&SwitchNode::m_ecnEnabled),
                          MakeBooleanChecker())
            .AddAttribute("CcMode", "CC mode.", UintegerValue(0), MakeUintegerAccessor(&SwitchNode::m_ccMode), MakeUintegerChecker<uint32_t>())
            .AddAttribute("AckHighPrio", "Set high priority for ACK/NACK or not", UintegerValue(0), MakeUintegerAccessor(&SwitchNode::m_ackHighPrio),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("MaxRtt", "Max Rtt of the network", UintegerValue(9000), MakeUintegerAccessor(&SwitchNode::m_maxRtt),
                          MakeUintegerChecker<uint32_t>());
    return tid;
}
...
SwitchNode::SwitchNode() {
    m_ecmpSeed = m_id;
    m_node_type = kNodeType::Switch;
    m_mmu = CreateObject<SwitchMmu>();
    for (uint32_t i = 0; i < pCnt; i++)
        for (uint32_t j = 0; j < pCnt; j++)
            for (uint32_t k = 0; k < qCnt; k++) m_bytes[i][j][k] = 0;
    for (uint32_t i = 0; i < pCnt; i++) m_txBytes[i] = 0;
    for (uint32_t i = 0; i < pCnt; i++) m_lastPktSize[i] = m_lastPktTs[i] = 0;
    for (uint32_t i = 0; i < pCnt; i++) m_u[i] = 0;
}
...
int SwitchNode::GetOutDev(Ptr<const Packet> p, CustomHeader &ch) {
    ...
    // no matching entry
    if (entry == m_rtTable.end()) return -1;
    ...
    if (idx >= 0) {
        ...
        if (qIndex != 0) {  // not highest priority
            if (m_mmu->CheckIngressAdmission(inDev, qIndex, p->GetSize()) &&
                m_mmu->CheckEgressAdmission(idx, qIndex, p->GetSize())) {  // Admission control
    ...
    if (1) {
        ...
        if (buf[PppHeader::GetStaticSize() + 9] == 0x11) {                               // udp packet
            ...
            if (m_ccMode == 3) {  // HPCC
                ...
                if (power > ih->GetPower()) ih->SetPower(power);
...
#else
        int mask = (1 << (msb - m)) - 1;
        if ((x0 & mask) > (rand() & mask)) x += 1 << (msb - m);
#endif
    }
